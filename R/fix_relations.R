...
#' @param replace_uid logical. If TRUE, replaces the UIDs in each relation with
#' the corresponding identifiers. If FALSE, just unnests the list.
#' @param uidlist Only needs to be provided if replace_UID = TRUE. A data.frame
#' as returned by `get_uid_list()`
#'
#' @return The resource with its relations unnested (and replaced with
#' identifiers if replace_uid is set to TRUE).
#'
#' @export
#'
#' @examples
fix_relations <- function(resource, replace_uid = TRUE, uidlist = NULL) {
