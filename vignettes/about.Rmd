...
The lists as they are usually returned by the database are not always very useful or easy to work with. The functions `simplify_idaifield()` aims to alleviate this somewhat by replacing all UIDs with identifiers, reformating the coordinates to make them usable, and spread columns from multiple choice fields. As the processing takes some time, I do not recommend using this function on the whole database if you have a large amount of resources. Instead, try using it on the results of queries or a selection of the database obtained with `select_by()`.
