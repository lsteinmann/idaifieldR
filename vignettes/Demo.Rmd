...
```{r}
layers <- select_by_type(idaifield_test_docs, type = "Layer")
layers[[5]]
```
Relations are still recorded as UIDs. Replacing them is easiest with `sapply()` and idaifieldR's `replace_uid()`-function, which needs a uidlist for reference:

```{r warning=FALSE}
layers[[5]] %>%
  sapply(function(x) replace_uid(x, uidlist = uidlist))
```
...
```{r}
pottery <- idaifield_test_docs %>%
  select_by_type(type = "Pottery") %>%
  idaifield_as_matrix()
  
pottery[1:5, 1:5]
```
(There is a function for data.frame conversion which pastes all entries from fields with multiple entries together, but I do not recommend it.)

With this matrix, it is easy to plot any distributions or actually further select and analyse the data. First, the UIDs should be replaced. If one wants to visualize the frequency of pottery finds over their containing objects, the values from "relations.liesWithin" are of interest:

```{r warning=FALSE}
pottery_df <- pottery %>%
  as.data.frame() %>%
  mutate(relations.liesWithin = replace_uid(relations.liesWithin,
                                            uidlist = uidlist))
...
```{r fig.width=7}
library(ggplot2)
ggplot(pottery_df, aes(x = relations.liesWithin)) +
  geom_bar()
```
...
ggplot(pottery_df, aes(x = relations.liesWithin, fill = materialType)) +
  geom_bar()
```
