...
If you wish to import the complete database into R, you can use `get_idaifield_docs()` with the default options. Be aware that this can take some time depending on the size of your database. Just to give you an impression: I regularly work with ca. 10000 resources, and the import takes roughly 5 seconds while the resulting `idaifield_resources`-object is around 40MB in size.
...
To make the list more usable and to fix the values of fields that may otherwise display UUIDs (as relations are stored this way in the database), you can use the function `simplify_idaifield()`. Depending on the size of you database it may take a while to process. You can decide whether you want to replace the UUIDs with their corresponding identifiers, and whether you want to use the geometry attached to each resource. Selecting a language will result in the function trying to retain the corresponding values in that language from multi-language text input fields.
...
ggplot(pottery, aes(x = condition)) +
  geom_bar()
```
You can do this with basically anything that is in the database. Some variables may need reformatting and cleaning, depending on your configuration and the state of the data. 
