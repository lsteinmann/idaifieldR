...
When importing data your first step would be to build a connection object, where serverip corresponds to the IP listet in the Field Client as "Eigene Adresse", user can be anything you wish and will only be important if you intend to write to the database, as the client does not employ user-specific passwords. pwd should be the password as listed in the Field-client.
