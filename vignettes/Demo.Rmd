...
idaifield_test_docs <- get_idaifield_docs(serverip = "192.168.1.21",
                                          # ^ The IP listet in the idaifield-
                                          # Client as "Eigene Adresse"
                                          projectname = "testproj",
                                          # ^ Name of the Project from said
                                          # client that should be imported
                                          user = "R",
                                          # ^ Username to be used
                                          # (inconsequential, as we are not
                                          # going to write to the db)
                                          # which would be possible with sofa,
                                          # btw, and may be useful in some case!
                                          pwd = "hallo",
                                          # ^ The password as listed in the
                                          # idaifield-Client
                                          simplified = TRUE
                                          # ^ default is TRUE, as it would
                                          # otherwise pull the docs list with
                                          # meta-info that I do not expect to
                                          # be needed in R, I nonetheless made
...
```{r}
pottery_df <- idaifield_as_df(pottery)
head(pottery_df)
```
...
library(dplyr)
# sorry got to fix that whitespace some time, not present in matrix though
#pottery_df$relations.liesWithin <- gsub("^\\s*", "", pottery_df$relations.liesWithin)
# i give up

pottery_df <- as.data.frame(pottery_mat)
match(pottery_df$relations.liesWithin, uidlist$UID)

pottery_df$relations.liesWithin <- as.character(pottery_df$relations.liesWithin)

pottery_df <- left_join(pottery_df, uidlist, by = c("relations.liesWithin" = "UID"),
                     suffix = c(".find", ".container"))
...
```{r fig.width=8}
library(ggplot2)
ggplot(pottery_df, aes(x = identifier.container)) +
  geom_bar()
```
...
ggplot(pottery_df, aes(x = identifier.container, fill = materialType)) +
  geom_bar()
```
