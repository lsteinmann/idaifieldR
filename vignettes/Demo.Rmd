...
all_resources <- simplify_idaifield(idaifield_test_docs)
...
`idaifieldR` contains a custom function to convert the list returned by the import functions `get_idaifield_docs()`, `idf_query()` and `idf_index_query()` into a matrix. For you convenience, you should probably convert this into a data.frame and try to assign the column type automatically: 
```{r eval=TRUE}
