...
The same outcome can be achieved by querying the database directly. This may be useful when you frequently want to update your data, but have a large amount of resources in the database: 
...
The query-functions will return `idaifield_docs`-lists that (if you so wish) you still need to simplify. You need to supply the index as "uidlist", otherwise UUIDs of relations cannot be replaced:

```{r querysimple1, eval = FALSE}
pottery_simple <- simplify_idaifield(pottery_docs, uidlist = index)
```
...
To get the translations from the Project configuration, you can use the config-attribute from any `idaifield_...`-list. This will also give you an impression of how the lookup-table is setup: 
