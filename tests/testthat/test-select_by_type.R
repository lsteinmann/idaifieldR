...
test_that("returns resource object", {
  expect_identical(class(select_by_type(test_resource, "Pottery")), "idaifield_resource")
})

test_that("returns less elements", {
  expect_lt(length(select_by_type(test_resource, "Pottery")), length(test_resource))
})
